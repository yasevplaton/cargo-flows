    ...
    map.addLayer({
      ...
      paint: {
        ...
        "line-offset": [
          ...
          ["zoom"],
          1,
          ["/", ["get", "bgLineOffset"], 512],
          2,
          ["/", ["get", "bgLineOffset"], 256],
          3,
          ["/", ["get", "bgLineOffset"], 128],
          4,
          ["/", ["get", "bgLineOffset"], 64],
          5,
          ["/", ["get", "bgLineOffset"], 32],
          ...
          ["*", ["get", "bgLineOffset"], 512],
          20,
          ["*", ["get", "bgLineOffset"], 1024],
          21,
          ["*", ["get", "bgLineOffset"], 2048],
          22,
          ["*", ["get", "bgLineOffset"], 4096]
        ]
      }
    });
  }
...
export function renderEdges(
  ...
  if (map.getSource("edges")) {
    ...
    reverseCargoArray.forEach(cargoObj => {
      map
        .getSource(`${cargoObj.type}-nodes`)
        .setData(multipleCargoNodesObject[cargoObj.type]);
    });
  } else {
    ...
    reverseCargoArray.forEach(cargoObj => {
      if (cargoObj.id === maxCargoId) {
        map.addSource(`${cargoObj.type}-nodes`, {
          type: "geojson",
          data: multipleCargoNodesObject[cargoObj.type]
        });
      ...
      map.addLayer({
        ...
        source: "edges",
        type: "line",
        filter: ["all", ["==", "type", cargoObj.type], ["!=", "value", 0]],
        paint: {
...
export function renderNodes(map, nodes, loadingClassArray) {
  if (map.getSource("nodes")) {
    ...
    map.addLayer({
      ...
      source: "nodes",
      type: "circle",
      filter: ["all", ["==", "name_rus", "junction"], [">", "cityRadius", 0]],
      layout: {
        visibility: "none"
      },
      paint: {
